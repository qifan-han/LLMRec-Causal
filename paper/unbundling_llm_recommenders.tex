...
\begin{abstract}
\noindent
AI agents powered by large language models (LLMs) are increasingly used by e-commerce platforms to guide consumers through product discovery, comparison, and choice. These systems differ from conventional recommenders because they generate a joint recommendation package: they select a product and produce language that explains why the product fits the consumer. We argue that this makes an LLM-backed shopping assistant a stochastic product-text recommendation system, whose causal effect should be decomposed into product-selection and expression channels. A standard A/B test of a backend policy change identifies the total effect of assigning consumers to one recommender, but it does not reveal whether the effect comes from better retrieval, more effective recommendation language, or the interaction between the two. We propose a modular factorial design that independently routes a policy shock to the retrieval stage and the expression stage, separately estimating the effects of changing product selection, changing recommendation text, and changing both together. In a semi-synthetic study with 30 real headphone products and 60 consumer personas, we apply a history-access shock to each stage separately. GPT-based synthetic evaluation reveals two large, opposing effects: routing historical information to product selection lowers recommendation quality through a premium-aspiration pattern, while routing the same information to text generation improves trust and tradeoff disclosure. The best-performing configuration is a hybrid that a bundled A/B test cannot observe. The framework provides e-commerce platforms with a practical diagnostic for identifying which layer of an LLM recommendation pipeline creates value.
\end{abstract}
...
This paper develops a causal decomposition framework for this setting. We model an LLM recommender as a stochastic policy over product-text pairs. The system observes a consumer query, a product catalog, and other context, then selects a product and generates recommendation text. Consumer response may depend on both objects: the same text can be more or less effective depending on the product it describes, and the same product can perform differently depending on whether the explanation is generic, personalized, transparent, or persuasive. A backend recommender shock, such as giving the system access to historical marketplace information, can therefore affect outcomes through product selection, through expression, or through their interaction.

We define a modular experimental design that separates these channels. The design routes the same recommender shock to two stages of an engineered LLM recommender. The first stage selects a product; the second stage writes the recommendation text conditional on the selected product. A binary indicator controls whether the shock is active in product selection, and a second indicator controls whether it is active in text generation. Crossing these two indicators creates four experimentally assigned regimes: a baseline recommender, a retrieval-only shock, an expression-only shock, and a full shock. The hybrid regimes, in which the shock is active in one stage but not the other, are precisely those that a standard one-shot policy change does not observe.

The resulting estimands have direct managerial interpretations. The retrieval effect asks what happens when the shock changes the product-selection stage while holding the expression policy fixed; the expression effect asks what happens when the shock changes the text-generation stage while holding the retrieval policy fixed; the interaction asks whether the two channels reinforce or offset each other. These are not post hoc mediation estimands based on realized text. They are contrasts between experimentally assigned policy components, following standard factorial-design logic applied to LLM recommendation \citep{egami2019causal, muralidharan2025factorial}. The decomposition matters because the total effect can be misleading: a full history-aware recommender may appear to have little value not because history is irrelevant, but because history improves one layer while damaging another. The point is analogous to recent work on actionable heterogeneity \citep{shchetkina2024actionable}: the presence of an LLM policy effect alone does not tell the firm how to improve the recommender. What matters is whether the effect comes from selection, expression, or their combination.
...
The remainder of the paper is structured as follows. Section~\ref{section:model} introduces the setting, notation, estimands, and identification. Section~\ref{section:empirics} describes the modular experimental architecture, the simulation design, and the data. Section~\ref{section:results} reports results. Section~\ref{section:conclusion} concludes.
...
The effects defined above can be estimated directly in an experiment. The identifying assumptions are standard for randomized policy experiments but are worth making explicit in this setting. First, the assigned recommender policies must be well defined: each value of $D_i$ or $(D_i^J,D_i^T)$ corresponds to a fixed stochastic generation protocol, including the model, prompt, catalog information, decoding settings, and stage-specific inputs. Because LLM outputs are stochastic, consistency is defined at the policy level rather than at the level of a deterministic product-text realization: an observed outcome under an assigned policy is one draw from the corresponding policy distribution. Second, assignment to the black-box policy or to the four modular cells must be randomized before recommendation generation, with every cell assigned with positive probability. Third, there is no interference across consumer sessions: the recommendation generated for one session does not affect the potential outcomes or generated recommendations for another session. Fourth, in the modular design, the expression stage must preserve the product selected by the retrieval stage; otherwise the $(D_i^J,D_i^T)$ contrast would no longer isolate a stage-specific routing of the treatment. These assumptions identify policy-component effects, not natural mediation effects of realized products or realized text. Under these assumptions, randomized assignment of $D_i$ identifies the black-box effect by the simple difference in average outcomes between the modified and baseline recommenders:
\begin{equation}
\tau^{BB} = \Expect[\Y_i \mid D_i = 1] - \Expect[\Y_i \mid D_i = 0].
\label{eq:identify-bb}
\end{equation}

Likewise, randomized assignment to the four modular cells identifies each modular policy mean:
\begin{equation}
...
%-------------------------------------------------------------------------
\section{Experimental Design and Empirical Strategy}\label{section:empirics}
%-------------------------------------------------------------------------

\subsection{Two-stage implementation}\label{section:design}
...
We call this pattern \emph{premium-aspiration bias}: history-aware retrieval shifts toward products with favorable qualitative feedback, even when those products exceed the consumer's stated budget. The pattern is distinct from popularity bias---the history-aware model does not shift toward bestsellers but toward products endorsed by qualitative sentiment such as ``strong post-purchase feedback among travelers'' or ``frequently chosen by audiophiles,'' which happen to be more expensive and less budget-appropriate for the requesting persona.
...
At baseline, both architectures draw on the same product specifications and generate similar recommendation packages. Under the history-access treatment, the unified system bundles retrieval and expression reasoning in a single call, while the modular system routes history to each stage with explicit control. The two-stage separation may produce better-targeted treated recommendations because the expression stage receives a locked product and can focus on communication rather than jointly reasoning about selection and language. The treated-condition advantage of the modular system is not evidence that two-stage recommenders are generally superior; it is specific to this product category, supply model, and information treatment. The main decomposition results should therefore be interpreted as effects within the experimentally modularized system. The black-box comparison is reassuring because the modular system is not visibly dominated by the unified assistant in this diagnostic, but it does not prove that the decomposition fully explains every black-box LLM recommender.

%-------------------------------------------------------------------------
\section{Conclusion}\label{section:conclusion}
%-------------------------------------------------------------------------

This paper develops a modular $2 \times 2$ experimental design that decomposes the effect of an LLM recommender policy change into a retrieval effect, an expression effect, and their interaction. Two findings stand out from the semi-synthetic study.

First, \emph{a bundled A/B test can miss opposing channel effects}. In our setting, the full history-aware treatment has a weakly negative total effect, but this net result masks two large, statistically significant, and opposing channels: retrieval harms while expression helps. The best-performing configuration---generic retrieval paired with history-aware expression---is a hybrid that a standard prompt A/B test never observes.

Second, \emph{information routing can matter as much as information access}. The same historical buyer information is valuable when routed to text generation, where it improves trust and tradeoff disclosure, but harmful when routed to product selection, where it triggers premium-aspiration bias. Giving an LLM recommender access to marketplace data without controlling how that data enters the recommendation pipeline can inadvertently degrade product-consumer fit.

A further analytical result is that realized-text regressions that score generated recommendations for endorsement strength and regress outcomes on that score can misattribute product-fit effects to language. The modular design avoids this problem by randomizing the expression policy before text generation rather than relying on variation in realized text.

The study also has several limitations that bound the conclusions above. All consumer outcomes are evaluated by GPT-5.3, not by human consumers; a human-subject experiment is a necessary next step. The findings come from one product category (headphones), one supply model (Qwen~2.5~14B), and one type of information shock (history access); replication across product domains, frontier models, and other policy contrasts would strengthen external validity. The design identifies effects only over the feasible recommendation support---products that neither retrieval policy nominates lie outside the estimand. We define the architecture gap $\Gamma = \tau^{BB} - \tau^{MO}$ conceptually and assess it through a qualitative pairwise diagnostic, but a formal quantitative estimator for $\Gamma$ is left for future work. Finally, the framework treats retrieval and expression as binary policy switches. Extending it to continuous or multi-level treatments and connecting it to optimal policy design is a natural direction.
...
This appendix validates the modular estimator against known ground truth in a fully parametric environment where the true retrieval, expression, and interaction effects are set by the researcher. The exercise serves two purposes: (i) confirming that the cell-mean contrasts recover the three components within sampling noise, and (ii) demonstrating quantitatively that the naive realized-expression regression can overstate the language channel at policy-relevant endogeneity levels.

We construct a structural data-generating process (DGP) with three hand-curated product catalogs (phone chargers, headphones, laptops) and 1{,}000 simulated consumers per category. Throughout this appendix, $q$ denotes the retrieval policy indicator (corresponding to $D_i^J$ in the main text), $r$ denotes the expression policy indicator ($D_i^T$), and $z$ denotes the black-box treatment ($D_i$). Each consumer-product pair has a latent fit score $\Q_{ij}$, and we simulate outcomes under two DGPs: an expression-leaning DGP (brand-forward retrieval, expression as leading channel) and a retrieval-dominant DGP (fit-aware retrieval). Under both DGPs, the modular design recovers the three components within sampling noise, while the naive realized-expression regression overstates the expression effect by 80--130\% at policy-relevant endogeneity levels. The structural validation is documented in detail below.
