...
\end{equation*}
In a field experiment, these quantities are identified by random assignment across consumer sessions. In controlled LLM exercises, we evaluate the same persona or query under the relevant policy conditions and compare outcomes within persona. This design does not require observing the LLM's internal reasoning. It only requires controlling which recommender version is used at each stage and observing the product, text, and outcome generated under each condition.
